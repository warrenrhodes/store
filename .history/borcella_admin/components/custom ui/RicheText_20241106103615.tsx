...
interface EditorElementProps {
  ...
  setElements: React.Dispatch<React.SetStateAction<StoredProductDescription[]>>;
}
...
export interface ProductDescriptionBuilderProps {
  initialData?:
    | { id: number; type: ElementType; content: string; url: string[] | null }[]
    | null;
...
const EditorElement: React.FC<EditorElementProps> = ({
  ...
  const handleFileUpload = useCallback(async (url: string, id: number) => {
    setElements((prev) =>
      prev.map((element) => {
        if (element.id === id) {
          return {
  ...
  const handleFileRemove = useCallback(async (url: string, id: number) => {
    setElements((prev) =>
      prev.map((element) => {
        if (element.id === id) {
          return {
            ...element,
            content: "",
            url: [...(element.url || []).filter((target) => target !== url)],
          };
        }
        return element;
      })
        ...
        {(element.type === "image" || element.type === "video") && (
          <div className="space-y-4">
            {element.type === "image" && (
              <ImageUpload
                value={element.url}
                onChange={(url) => handleFileUpload(url, element.id)}
                onRemove={(url) => handleFileRemove(url, element.id)}
              />
            )}
