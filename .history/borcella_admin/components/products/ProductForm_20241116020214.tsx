    ...
    controls: {
      ...
      ol: {
        ...
        exec: (editor: any, current: any, { control, button }: any) => {
          ...
          if (current && current.parentElement) {
            editor.selection.save();
            const ul = editor.selection.closest(current, "ol,ul");
      ...
      ul: {
        ...
        exec: (editor: any, current: any, { control, button }: any) => {
          ...
          if (current && current.parentElement) {
            editor.selection.save();
            const ol = editor.selection.closest(current, "ol,ul");

            if (ol) {
              if (ol.tagName.toLowerCase() === "ul") {
                ol.style.listStyleType = currentValue;
              } else {
                const ul = editor.createInside.element("ul");
                ul.style.listStyleType = currentValue;
                ol.parentNode.replaceChild(ul, ol);
                Array.from(ol.children).forEach((li: any) =>
