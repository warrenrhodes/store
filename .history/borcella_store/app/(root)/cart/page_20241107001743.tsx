    ...
    <div className="flex gap-20 py-16 px-10 max-lg:flex-col max-sm:px-3">
          ...
          <div>
            {cart.cartItems.map((cartItem) => (
              <div className="w-full flex max-sm:flex-col max-sm:gap-3 hover:bg-grey-1 px-4 py-3 items-center max-sm:items-start justify-between">
                ...
                >
                  <Image
                    src={`${process.env.NEXT_PUBLIC_SERVER_URL}${cartItem.item.media[0].url}`}
                    width={100}
                    height={100}
      ...
      <div className="w-1/3 max-lg:w-full flex flex-col gap-8 bg-grey-1 rounded-lg px-4 py-5">
        ...
        <div className="flex justify-between text-body-semibold">
          ...
          <span>FCFA {totalRounded}</span>
        </div>
        <button
          className="border rounded-lg text-body-bold bg-black py-3 w-full hover:bg-black/80 text-white"
