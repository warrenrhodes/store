...
const RelatedProduct = async ({ product }: { product: ProductType }) => {
  'use server';
  const reviews = await getReviewByProductId(product._id);
  ...
  const ratings = productReviews ? calculateRating(
