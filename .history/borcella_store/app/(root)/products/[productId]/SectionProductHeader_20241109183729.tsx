...
const Promotions = ({ promotions }: { promotions: PromotionType[] }) => {
  ...
  const cart = useCart();
  const cartItems = cart.cartItems.find((item) => item.item.title === title);
  const handlePromotionUpdated = (promo: PromotionType) => {
    if ((cartItems?.quantity || 1) < promo.minQuantity) {
      toast.error(
        `The minimum quantity to validate the promotion  is ${promo.minQuantity}`
      );
      return;
    }
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-col flex-1">
                    <p className="text-sm font-bold">
                      {promo.minQuantity} {promo.promotionName}
                    </p>
