...
const CartSideBar: React.FC<CartSideBarProps> = () => {
  ...
  const total = cart.cartItems.reduce(
    (
      ...
      cartItem: {
        item: { price: number };
        quantity: number;
        promotion?: { currentPrix: number };
      }
    ) =>
      acc +
      (cartItem.promotion?.currentPrix || cartItem.item.price) *
        cartItem.quantity,
    0
  );
  ...
  const calculateTotalPrice = (
    ...
    if (!selectedPromotion || quantity === 0) {
      return product.currentPrix * quantity;
    }

    const promotionApplications = Math.floor(
      quantity / selectedPromotion.minQuantity
    );
    const remainingItems = quantity % selectedPromotion.minQuantity;

    const promotionTotal =
      promotionApplications * selectedPromotion.currentPrix;
    const remainingTotal = remainingItems * product.currentPrix;

    return promotionTotal + remainingTotal;
  };
