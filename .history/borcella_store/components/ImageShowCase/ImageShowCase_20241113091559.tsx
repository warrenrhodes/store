...
const ProductGallery: FC<ImageShowCaseProps> = ({ medias }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <div className="relative h-96 md:h-[600px] bg-gray-50 rounded-lg overflow-hidden">
        <AnimatePresence initial={false} custom={direction}>
          <motion.img
            ...
            className="absolute w-full h-full object-contain duration-500"
          />
        </AnimatePresence>
        ...
        <button
          className="absolute z-10 right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/80 hover:bg-white shadow-lg transition-all"
          onClick={() => paginate(1)}
        >
      ...
      <div className="mt-4 flex gap-2 overflow-x-auto p-3 items-center">
        {medias.map((image, index) => (
          <button
            ...
            className={cn(
              {
                "ring-2 ring-primary": currentImage === index,
                "hover:opacity-75": currentImage !== index,
              },
