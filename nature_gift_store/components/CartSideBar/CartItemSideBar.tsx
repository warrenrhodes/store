...
export const CartItemSideBarComponent = ({ cartItem }: { cartItem: CartItem }) => {
  const cart = useCart()
  const min = 1
  const max = 99
  const { item } = cartItem
  const { _id, title, media, categories, price: expense, promoPrice: price } = item
      ...
      <div className="ml-4 flex flex-1 flex-col justify-between gap-2">
        <div>
          <div className="flex justify-between ">
            ...
            <div>
              <p className="line-through text-sm text-red-600">FCFA {expense}</p>
