...
interface IPromotionAction {
  type: 'PERCENTAGE_DISCOUNT' | 'FIXED_DISCOUNT' | 'FREE_SHIPPING' | 'FREE_PRODUCT' | 'BUY_X_GET_Y'
  value: number | string | string[]
  maxDiscount?: number
}

interface IPromotionCondition {
  type: 'MINIMUM_QUANTITY' | 'SPECIFIC_PRODUCTS' | 'DELIVERY_METHOD' | 'LOCATION'
  value: number | string | string[]
}
...
const promotionActionSchema = new Schema({
  type: {
    type: String,
    enum: ['PERCENTAGE_DISCOUNT', 'FIXED_DISCOUNT', 'FREE_SHIPPING', 'FREE_PRODUCT', 'BUY_X_GET_Y'],
    required: true,
  },
  value: {
...
const promotionConditionSchema = new Schema({
  type: {
    type: String,
    enum: ['MINIMUM_QUANTITY', 'SPECIFIC_PRODUCTS', 'DELIVERY_METHOD', 'LOCATION'],
    required: true,
  },
  value: {
    type: Schema.Types.Mixed,
